...
import { buildExecutiveViewModel, buildSystemHealthViewModel, buildAutomationViewModel, buildReleaseReadinessViewModel, createRequestCache } from "../utils/enterpriseUiIntegration.js";
...
export default function Dashboard({ onOpenDealIntake, onOpenDealAnalyzer, onOpenFlipAnalyzer, onOpenBrrrrAnalyzer, onOpenProductVault, onOpenContractorHub, onOpenDealIntelligence, onOpenCompDatabase, onOpenNeighborhoodDatabase, onOpenPortfolioDashboard, onOpenPropertyDatabase, onOpenVendorDatabase, onOpenMaterialMatrix, onOpenLenderDashboard, onOpenAppraiserPacketBuilder, onOpenRehabProjectTracker }) {
  ...
  useEffect(() => {
    const loadSourceData = async () => {
      try {
        const [dealsRes, intelligenceRes, propertiesRes, rehabRes, contractorsRes, lendersRes, portfolioRes, healthRes] = await Promise.all([
        ...
        const [deals, dealIntelligence, properties, rehabProjects, contractors, lenders, portfolioData, healthPayload] = await Promise.all([
          readJson(dealsRes),
        ...
        const operationsDerivedSummary = {
          ...
          alertCount: (derivedLiveModel?.operationsAlerts?.length || 0) + (derivedOperationsSummary?.alertCount || 0),
        };
  ...
  const operationsHighlights = useMemo(() => ({
    ...
    alertCount: Array.isArray(enterpriseViewModel?.operationsAlerts) ? enterpriseViewModel.operationsAlerts.length : 0,
  ...
  const releaseReadinessSummary = useMemo(() => buildReleaseReadinessViewModel({
    backendHealth: systemHealth || { healthy: true },
    operationsSummary,
    automationSummary,
    portfolioIntelligence: intelligence.dataSources.portfolioIntelligence,
    systemHealth,
  }), [automationSummary, intelligence.dataSources.portfolioIntelligence, operationsSummary, systemHealth]);
  ...
  const executivePriorities = useMemo(() => {
    const executiveViewModel = buildExecutiveViewModel({ deals: intelligence.dataSources.deals, portfolioIntelligence: intelligence.dataSources.portfolioIntelligence, backendHealth: systemHealth || { healthy: true } });
    const priorities = [...(intelligence.priorities || [])];
    if (enterpriseViewModel?.workflowSnapshot?.summary) {
      priorities.unshift({
        priority: 'AUTOMATION',
        action: `${enterpriseViewModel.workflowSnapshot.summary.completedModuleCount} workflow modules active · ${enterpriseViewModel.workflowSnapshot.summary.overallHealthScore}/100`,
      });
    }
    ...
    if (enterpriseViewModel?.portfolioOverview?.executiveSummary?.headline) {
      priorities.unshift({
        priority: "PORTFOLIO",
        action: enterpriseViewModel.portfolioOverview.executiveSummary.headline,
      });
    }
    if (executiveViewModel?.highestPriorityAction) {
      priorities.unshift({ priority: "EXECUTIVE", action: executiveViewModel.highestPriorityAction });
    }
    if (aiDecisionEngine?.dealDecision) {
      priorities.unshift({
    ...
    return priorities;
  }, [aiDecisionEngine, capitalAllocationEngine, enterpriseViewModel, intelligence.priorities, systemHealth]);
      ...
      <main style={styles.main}>
        ...
        <section style={styles.metrics}>
          {(enterpriseViewModel?.topMetrics || topMetrics).map((metric) => (
            <div key={metric.title} style={styles.metric}>
        ...
        <section style={styles.intelligenceStrip}>
          {(enterpriseViewModel?.intelligenceCards || []).map((card) => (
            <div key={card.label} style={styles.intelligenceCard}><div style={styles.intelligenceLabel}>{card.label}</div><div style={styles.intelligenceValue}>{card.value}</div></div>
          ))}
        </section>
        ...
        <section style={styles.operationsRow}>
          ...
          <div style={styles.operationsCard}>
            <div style={styles.intelligenceLabel}>PORTFOLIO HEALTH</div>
        ...
        <section style={styles.operationsRow}>
          ...
          <div style={styles.operationsCard}>
            ...
            <div style={styles.intelligenceValue}>{(operationsHighlights.alertCount || (enterpriseViewModel?.summaryStats?.totalAlertCount ?? operationsSummary?.alertCount ?? 0))}</div>
          </div>
          <div style={styles.operationsCard}>
        ...
        <section style={styles.operationsRow}>
          <div style={styles.operationsCard}>
            ...
            <div style={styles.intelligenceValue}>{enterpriseViewModel?.workflowSnapshot?.summary?.overallHealthScore ? `${enterpriseViewModel.workflowSnapshot.summary.overallHealthScore}/100` : automationSummary?.summaryLabel || 'Pending · Pending'}</div>
          </div>
          <div style={styles.operationsCard}>
