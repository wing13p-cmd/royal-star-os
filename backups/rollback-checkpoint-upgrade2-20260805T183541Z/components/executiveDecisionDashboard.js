...
function collectBlockingItems(deal, analysis, scenarioAnalysis, redTeamReview) {
  const items = [];
  ...
  if (safeDisplay(redTeamReview?.recommendationSurvivalResult, "Insufficient Data") === "Fails") {
    items.push({ item: "Red-Team failure", reason: "The recommendation does not survive Red-Team review.", owner: "Deal Analyzer", priority: "Decision Blocking", dueStatus: "Pending", resolutionNeeded: "Rework the assumptions." });
  }
  return items.slice(0, 5);
}

function collectNextActions(deal, analysis, scenarioAnalysis, redTeamReview) {
  const actions = [];
  if (safeDisplay(analysis.arvConfidence, "Insufficient Data") === "Very Low" || safeDisplay(analysis.arvConfidence, "Insufficient Data") === "Low") {
...
function buildKnownUncertainNeeded(deal, analysis, scenarioAnalysis, redTeamReview) {
  const known = [];
...
export function buildExecutiveDecisionDashboard(primaryDealAnalysis = {}, primaryDeal = {}, scenarioAnalysis = {}, redTeamReview = {}, allDealRecords = [], allDeals = []) {
  try {
    ...
    const blockingItems = collectBlockingItems(deal, analysis, scenario, redTeam);
    const nextActions = collectNextActions(deal, analysis, scenario, redTeam);
    const decisionMatrix = buildDecisionMatrix(deal, analysis, scenario, redTeam);
    const knownUncertainNeeded = buildKnownUncertainNeeded(deal, analysis, scenario, redTeam);
