...
async function readDealsFile() { return readJsonArrayFile(dealsFile, [], "deals"); }
async function writeProductsFile(products) { await writeJsonFile(productsFile, products); }
async function readProductsFile() { return readJsonArrayFile(productsFile, [], "products"); }
async function writeContractorsFile(contractors) { await writeJsonFile(contractorsFile, contractors); }
async function readContractorsFile() { return readJsonArrayFile(contractorsFile, [], "contractors"); }
...
async function readPortfolioFile() { return readJsonArrayFile(portfolioFile, [], "portfolio"); }
...
async function readMaterialsFile() { return readJsonArrayFile(materialsFile, [], "materials"); }
async function writeLendersFile(lenders) { await writeJsonFile(lendersFile, lenders); }
async function readLendersFile() { return readJsonArrayFile(lendersFile, [], "lenders"); }
async function writeAppraisalPacketsFile(packets) { await writeJsonFile(appraisalPacketsFile, packets); }
async function readAppraisalPacketsFile() { return readJsonArrayFile(appraisalPacketsFile, [], "appraisal packets"); }
async function writeRehabProjectsFile(projects) { await writeJsonFile(rehabProjectsFile, projects); }
async function readRehabProjectsFile() { return readJsonArrayFile(rehabProjectsFile, [], "rehab projects"); }
...
async function handleCollection(req, res, entity, readFile, writeFile, normalizeFn, validateFn, newIdPrefix) {
  ...
  if (req.method === "POST" && !itemId) {
    ...
    const persistedPayload = entity === "deal" ? buildPersistedDealPayload(payload) : payload;
...
server.listen(port, "127.0.0.1", async () => {
  ...
  console.log(`[RSOS] Backend listening on http://127.0.0.1:${port}`);
});
