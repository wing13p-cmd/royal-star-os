...
const compQualityOptions = ["Excellent", "Good", "Fair", "Weak", "Reject"];
const sortOptions = [
  ["name", "Packet Name"],
  ["newest", "Newest"],
  ["oldest", "Oldest"],
...
function buildCompRecords(packetComps, basePacket) {
  return (packetComps || []).map((comp, index) => ({
    ...
    compWeight: comp.compWeight || 1,
    qualityGrade: comp.qualityGrade || "Fair",
    adjustmentAmount: comp.adjustmentAmount || "",
    adjustedSalePrice: comp.adjustedSalePrice || comp.salePrice || "",
    notes: comp.notes || "",
  }));
}

function computeCompMetrics(comp) {
  ...
  const weightContribution = Number(comp.compWeight || 1) * Number(adjustedSalePrice || 0);
  const saleDate = comp.saleDate ? new Date(comp.saleDate) : null;
  const now = new Date();
  const saleAgeInDays = saleDate && !Number.isNaN(saleDate.getTime()) ? Math.max(0, Math.round((now.getTime() - saleDate.getTime()) / (1000 * 60 * 60 * 24))) : "";
  return { pricePerSquareFoot, adjustedSalePrice, weightContribution, saleAgeInDays };
}
...
export default function AppraiserPacketBuilder({ onBack, onOpenDealAnalyzer, onOpenFlipAnalyzer, onOpenBrrrrAnalyzer, onOpenProductVault, onOpenContractorHub, onOpenCompDatabase, onOpenDealIntelligence, onOpenNeighborhoodDatabase, onOpenPortfolioDashboard, onOpenPropertyDatabase, onOpenVendorDatabase, onOpenMaterialMatrix, onOpenLenderDashboard, onOpenDealIntake }) {
  ...
  const loadPackets = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/appraisal-packets`);
  ...
  const removeArrayItem = (name, index) => {
    setFormValues((current) => ({ ...current, [name]: (current[name] || []).filter((_, itemIndex) => itemIndex !== index) }));
  };
  ...
  const savePacket = async (event) => {
    ...
    try {
      const response = editingId
        ? await fetch(`${API_BASE_URL}/api/appraisal-packets/${editingId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) })
        : await fetch(`${API_BASE_URL}/api/appraisal-packets`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      if (!response.ok) throw new Error("backend unavailable");
  ...
  const deletePacket = async (packetId) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/appraisal-packets/${packetId}`, { method: "DELETE" });
      if (!response.ok) throw new Error("backend unavailable");
  ...
  const toggleFavorite = async (packet) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/appraisal-packets/${packet.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(updated) });
      if (!response.ok) throw new Error("backend unavailable");
      const saved = await response.json();
  ...
  const importDeal = (dealId) => {
    ...
    setFormValues((current) => ({
      ...
      requestedARV: current.requestedARV || deal.estimatedArv || "",
  ...
  const updateCompForPacket = (compId, field, value) => {
    const nextPackets = packets.map((packet) => {
      if (packet.id !== formValues.id) return packet;
      return { ...packet, comps: (packet.comps || []).map((comp) => (comp.id === compId ? { ...comp, [field]: value } : comp)) };
    });
    setPackets(nextPackets);
    setFormValues((current) => ({ ...current, comps: (current.comps || []).map((comp) => (comp.id === compId ? { ...comp, [field]: value } : comp)) }));
  };
  ...
  return (
    <div style={styles.page}>
      ...
      <main style={styles.main}>
        ...
        {printView ? <section style={styles.printView} className="print-only"><div style={styles.printHeader}>APPRAISER PACKET</div>{visiblePackets[0] ? <div><h2>{visiblePackets[0].packetName}</h2><p>{visiblePackets[0].address}</p><p>{visiblePackets[0].propertySummary}</p></div> : <div>No packet selected.</div>}</section> : null}
      </main>
    </div>
  );
}
