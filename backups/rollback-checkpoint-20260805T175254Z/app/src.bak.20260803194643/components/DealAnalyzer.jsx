...
import { buildUnifiedUnderwritingIntelligence, normalizeDealForIntelligence, buildUnderwritingMetrics } from "./intelligenceUpgradeEngine.js";
...
function createStorageDeal(deal) {
  return {
    ...normalizeDeal(deal),
    address: deal.address || deal.propertyAddress || deal.address || "",
    propertyAddress: deal.propertyAddress || deal.address || "",
    zipCode: deal.zipCode || deal.zip || "",
    zip: deal.zip || deal.zipCode || "",
    arv: deal.arv ?? deal.estimatedArv ?? "",
...
export default function DealAnalyzer({ onBack, onOpenDealIntake, onOpenDealIntelligence, onEditDeal }) {
  ...
  const aiDecisionInsight = useMemo(() => {
    ...
    const analysis = {
      dealScore: Math.max(0, Math.min(100, Math.round(Math.max(0, underwriting.flipAnalysis.netProfit > 0 ? 75 : 40) + (underwriting.buyBox.decision === "Strong Pass" ? 10 : 0)))) ,
      financingScore: 72,
      overallRisk: underwriting.buyBox.decision === "Strong Pass" ? 20 : underwriting.recommendation.action === "PROCEED" ? 35 : 55,
                ...
                <div style={styles.detailPanel}>
                  ...
                  <div style={styles.detailGrid}>
                    {Object.entries({
                      ...
                      ProjectedProfit: formatCurrency(toNumber(aiDecisionInsight?.evidenceModel ? undefined : 0)),
