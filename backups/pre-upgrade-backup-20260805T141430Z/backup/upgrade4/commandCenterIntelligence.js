...
function buildAlerts(deals, properties, rehabProjects, contractors, lenders, portfolioIntelligence) {
  ...
  dealList.forEach((deal) => {
    ...
    const arv = safeNumber(deal.estimatedArv || deal.arv);
  ...
  const portfolioAlert = portfolioIntelligence?.alerts?.find((alert) => alert.alert === 'Negative equity' || alert.alert === 'Reserve shortfall');
  if (portfolioAlert) {
    alerts.push({ severity: portfolioAlert.severity || 'HIGH', alert: portfolioAlert.alert, relatedRecord: safeString(portfolioAlert.property || 'Portfolio', 'Portfolio'), financialExposure: safeString(portfolioAlert.financialExposure || 'Insufficient Data'), requiredAction: safeString(portfolioAlert.requiredAction || 'Review portfolio risk'), relatedModule: safeString(portfolioAlert.relatedModule || 'Portfolio Dashboard'), status: 'Open' });
  }
...
function buildSearchResults(query, deals, properties, contractors, lenders, dealIntelligence, alerts) {
  ...
  normalizeArray(deals).forEach((deal) => {
    ...
    if (haystack.includes(search)) results.push({ label: safeString(deal.propertyAddress, 'Deal'), module: 'Deal Analyzer' });
  });
  normalizeArray(properties).forEach((property) => {
    ...
    if (haystack.includes(search)) results.push({ label: safeString(property.propertyName || property.address, 'Property'), module: 'Property Database' });
  });
  normalizeArray(contractors).forEach((contractor) => {
    if (safeString(contractor.contractorName).toLowerCase().includes(search)) results.push({ label: safeString(contractor.contractorName, 'Contractor'), module: 'Contractor Hub' });
  });
  normalizeArray(lenders).forEach((lender) => {
    if (safeString(lender.lenderName).toLowerCase().includes(search)) results.push({ label: safeString(lender.lenderName, 'Lender'), module: 'Lender Dashboard' });
  });
  normalizeArray(dealIntelligence).forEach((entry) => {
    if (safeString(entry.recommendation || entry.decision).toLowerCase().includes(search)) results.push({ label: safeString(entry.analysisName || entry.recommendation || entry.decision, 'Analysis'), module: 'Deal Intelligence' });
  });
  normalizeArray(alerts).forEach((alert) => {
    if (safeString(alert.alert).toLowerCase().includes(search)) results.push({ label: safeString(alert.alert, 'Alert'), module: safeString(alert.relatedModule, 'Portfolio Dashboard') });
  });
...
export function buildCommandCenterIntelligence(payload = {}) {
  ...
  const alerts = buildAlerts(deals, properties, rehabProjects, contractors, lenders, portfolioIntelligence);
  const priorities = buildPriorities(deals, properties, rehabProjects, lenders, portfolioIntelligence);
  const topOpportunity = buildTopOpportunity(deals, dealIntelligence, properties, portfolioIntelligence);
  const topRisk = buildTopRisk(properties, portfolioIntelligence, rehabProjects);
  ...
  const searchResults = buildSearchResults('', deals, properties, contractors, lenders, dealIntelligence, alerts);
  const syncSummary = buildSyncSummary(syncState);

  return {
    businessStatus: determineBusinessStatus({ portfolioHealthScore: portfolioIntelligence?.summary?.healthScore }, portfolioIntelligence),
    portfolioHealth: safeDisplay(portfolioIntelligence?.health?.status || portfolioIntelligence?.summary?.healthStatus || portfolioIntelligence?.summary?.healthGrade, 'Insufficient Data'),
    pipelineHealth: safeDisplay(portfolioIntelligence?.health?.status || portfolioIntelligence?.summary?.healthStatus, 'Insufficient Data'),
