...
test('getRequestContext accepts a matching bearer token as admin access', () => {
  const context = getRequestContext(
    { headers: { host: 'example.com', authorization: 'Bearer rsos-admin-token' } },
    { env: { RSOS_ADMIN_TOKEN: 'rsos-admin-token' } },
  );

  assert.equal(context.isAdmin, true);
  assert.equal(hasPermission(context, 'write'), true);
});

test('protected-route authorization denies remote writes without role context', () => {
  const context = getRequestContext({ headers: { host: 'example.com' } });
  assert.equal(context.isAdmin, false);
  assert.equal(hasPermission(context, 'write'), false);
});
