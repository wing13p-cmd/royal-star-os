...
function getPortfolioMetrics(item) {
  ...
  const operatingExpenses = Number(item.operatingExpenses || 0);
  const annualTaxes = Number(item.annualTaxes || 0);
  const annualInsurance = Number(item.annualInsurance || 0);
  const loanBalance = Number(item.loanBalance || 0);
  const monthlyDebtService = Number(item.monthlyDebtService || 0);
  const occupancyRate = Number(item.occupancyRate || 0);
  const annualGrossRent = monthlyRent * 12;
  const annualNetOperatingIncome = annualGrossRent * (occupancyRate / 100) - operatingExpenses - annualTaxes - annualInsurance;
  const cashFlow = annualNetOperatingIncome - (monthlyDebtService * 12);
  const equity = currentValue - loanBalance;
  const capRate = annualGrossRent > 0 && currentValue > 0 ? (annualNetOperatingIncome / currentValue) * 100 : 0;
  const roi = purchasePrice > 0 ? ((currentValue - purchasePrice) / purchasePrice) * 100 : 0;
  const risk = equity < 0 ? "High" : capRate < 4 ? "Watch" : "Healthy";
...
export default function PortfolioDashboard({
  ...
  const [portfolioIntelligence, setPortfolioIntelligence] = useState(null);
  ...
  const filteredPortfolio = useMemo(() => {
    ...
    const filtered = normalizedPortfolio.filter((item) => {
      ...
      const matchesFavorite = favoriteFilter === "All" || (favoriteFilter === "Favorites" ? Boolean(item.favorite) : !Boolean(item.favorite));
  ...
  const summaryStats = useMemo(() => {
    ...
    const averageCapRate = total > 0 ? normalizedPortfolio.reduce((sum, item) => sum + Number(item.metrics.capRate || 0), 0) / total : 0;
    const topRoi = [...normalizedPortfolio].sort((left, right) => Number(right.metrics.roi || 0) - Number(left.metrics.roi || 0))[0] || null;
    const favorites = normalizedPortfolio.filter((item) => Boolean(item.favorite)).length;
  ...
  useEffect(() => {
    const intelligence = buildPortfolioIntelligence(normalizedPortfolio, [], [], [], [], [], [], []);
  ...
  const handleFavoriteToggle = async (propertyId) => {
    ...
    const nextPortfolio = portfolio.map((item) => (item.id === propertyId ? { ...item, favorite: !Boolean(item.favorite) } : item));
    setPortfolio(nextPortfolio);
    if (typeof window !== "undefined") {
      window.localStorage.setItem("royalStarPortfolio", JSON.stringify(nextPortfolio));
    }
    ...
    try {
      await fetch(buildApiUrl(`/api/portfolio/${propertyId}`), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...target, favorite: !Boolean(target.favorite) }),
      });
    } catch (error) {
  ...
  const handleImportFromDeal = async () => {
    try {
      ...
      if (!Array.isArray(deals) || deals.length === 0) {
        setMessage({ type: "error", text: "No deals are available to import yet." });
        return;
      }
      const sourceDeal = deals[0];
      setFormValues((prev) => ({
        ...prev,
        propertyName: sourceDeal.propertyAddress || prev.propertyName,
        propertyAddress: sourceDeal.propertyAddress || prev.propertyAddress,
        city: sourceDeal.city || prev.city,
        state: sourceDeal.state || prev.state,
        zipCode: sourceDeal.zipCode || prev.zipCode,
        purchasePrice: sourceDeal.purchasePrice || prev.purchasePrice,
        rehabBudget: sourceDeal.rehabBudget || prev.rehabBudget,
        notes: sourceDeal.notes ? `Imported from deal: ${sourceDeal.notes}` : prev.notes,
      }));
      setMessage({ type: "success", text: "Deal details imported into the portfolio form." });
    } catch (error) {
      ...
      <main style={styles.main}>
        ...
        <section style={styles.summaryPanel}>
          ...
          <div style={styles.summaryCards}>
            <SummaryCard label="Properties" value={summaryStats.total} />
            ...
            <SummaryCard label="Health Score" value={portfolioIntelligence?.summary?.healthScore ? `${portfolioIntelligence.summary.healthScore} / 100` : "Insufficient Data"} />
            ...
            <SummaryCard label="Reserve Shortfall" value={portfolioIntelligence?.summary?.reserveShortfallValue ? formatCurrency(portfolioIntelligence.summary.reserveShortfallValue) : "—"} />
                ...
                {filteredPortfolio.map((item) => (
                  <div key={item.id} style={styles.listItem}>
                    <div style={styles.listHeader}>
                      ...
                      <div style={{ ...styles.ratingPill, borderColor: item.metrics.risk === "Healthy" ? "#2f7d32" : item.metrics.risk === "Watch" ? "#b68a1b" : "#9c2a2a" }}>{item.metrics.risk}</div>
                    </div>
