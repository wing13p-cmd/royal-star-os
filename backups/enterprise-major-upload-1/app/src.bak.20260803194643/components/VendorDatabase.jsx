...
const riskLevelOptions = ["All", "Low", "Moderate", "High", "Critical"];
const sortOptions = [
  ["name", "Vendor Name"],
...
export default function VendorDatabase({
  ...
  const [importedProductVendor, setImportedProductVendor] = useState("");
  ...
  const handleToggleFavorite = (vendorId) => {
    const nextVendors = vendors.map((vendor) => (vendor.id === vendorId ? { ...vendor, favorite: !Boolean(vendor.favorite) } : vendor));
    setVendors(nextVendors);
    if (typeof window !== "undefined") {
      window.localStorage.setItem("royalStarVendors", JSON.stringify(nextVendors));
    }
