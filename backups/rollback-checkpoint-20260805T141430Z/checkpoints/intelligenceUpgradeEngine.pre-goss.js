...
export function buildUnderwritingMetrics(deal = {}, financing = {}, options = {}) {
  ...
  const profit = grossProfit - sellingCosts;
...
export function buildArvIntelligence(deal = {}, comps = [], neighborhoods = []) {
  ...
  const baseArv = manualArvOverride > 0 ? manualArvOverride : weightedAdjustedArv;
...
export function buildInvestmentDecisionEngine(deal = {}, analysis = {}) {
  ...
  if (buyBoxPass && dealScore >= 80 && lowRisk && highConfidence && strongEconomics && qualified && warnings.length === 0) {
  ...
  } else if (estimatedProfit <= 0 || roi <= 0 || cashRequired > safeNumber(normalizedDeal.purchasePrice || normalizedDeal.askingPrice || 0) * 1.1) {
    recommendation = "Renegotiate";
    confidence = 74;
    reasons = ["The current pricing or cost structure is too aggressive.", "Renegotiating the purchase price or scope is the most responsible course."];
  } else {
    recommendation = "Pass";
    confidence = 66;
...
export function buildSharedUnderwritingSnapshot(deal = {}, comps = [], neighborhoods = []) {
  const normalizedDeal = normalizeDealForIntelligence(deal);
  const financing = { loanAmount: safeNumber(normalizedDeal.loanAmount || normalizedDeal.financingLoanAmount || 0) };
  const metrics = buildUnderwritingMetrics(normalizedDeal, financing, { includeContingency: true, includeHoldingCost: true, includeTaxesAndInsurance: false, includeExtraCosts: false });
  ...
  const summary = {
    ...
    totalProjectCost: underwriting.flipAnalysis?.totalProjectCost ?? metrics.totalProjectCost,
    arv: underwriting.arvAnalysis?.supportedBaseArv ?? metrics.arv,
    profit: underwriting.flipAnalysis?.netProfit ?? metrics.profit,
    roi: metrics.roi,
...
export function buildUnifiedUnderwritingIntelligence(deal = {}, comps = [], neighborhoods = []) {
  ...
  const flipAnalysis = {
    purchasePrice,
    closingCosts: safeNumber(normalizedDeal.closingCosts || 0),
    rehab: rehabBudget,
    contingency,
    financingCosts: safeNumber(normalizedDeal.financingCosts || 0),
    holdingCosts: safeNumber(normalizedDeal.holdingCosts || 0),
    ...
    profitMargin: arvAnalysis.supportedBaseArv > 0 ? metrics.profit / arvAnalysis.supportedBaseArv : 0,
    returnOnCost: metrics.totalProjectCost > 0 ? metrics.grossProfit / metrics.totalProjectCost : 0,
    breakEvenSalePrice: metrics.totalProjectCost + metrics.sellingCosts,
  ...
  const cashInvested = purchasePrice + rehabBudget + contingency + safeNumber(normalizedDeal.closingCosts || 0) + safeNumber(normalizedDeal.financingCosts || 0);
  ...
  const recommendation = {
    action: flipAnalysis.netProfit > 0 && cashLeftInDeal > 0 ? "PROCEED" : "REQUEST MORE DATA",
    ...
    nextAction: "Complete inspection and confirm comp support.",
  };
...
export function buildEnterpriseDealIntelligenceSummary(deal = {}, analysis = {}, deals = [], properties = [], contractors = [], lenders = [], dealIntelligence = [], alerts = [], appraisalPackets = [], portfolioIntelligence = {}, lessons = []) {
  ...
  return {
    ...
    commandRouting,
    summary: {
      primaryRecommendation: commandRouting.route || search.recommendedModule || "Deal Intelligence",
      confidence: safeString(knowledge.confidence, "Insufficient Data"),
      recommendationReason: safeString(commandRouting.rationale || knowledge.recommendedNextInquiry, "Insufficient Data"),
