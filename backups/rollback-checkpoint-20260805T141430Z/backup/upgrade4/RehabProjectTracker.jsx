...
export default function RehabProjectTracker({ onBack, onOpenDealAnalyzer, onOpenFlipAnalyzer, onOpenBrrrrAnalyzer, onOpenProductVault, onOpenContractorHub, onOpenCompDatabase, onOpenDealIntelligence, onOpenNeighborhoodDatabase, onOpenPortfolioDashboard, onOpenPropertyDatabase, onOpenVendorDatabase, onOpenMaterialMatrix, onOpenLenderDashboard, onOpenAppraiserPacketBuilder, onOpenDealIntake }) {
  ...
  const loadProjects = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/rehab-projects`);
  ...
  const saveProject = async (event) => {
    ...
    try {
      const response = editingId
        ? await fetch(`${API_BASE_URL}/api/rehab-projects/${editingId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) })
        : await fetch(`${API_BASE_URL}/api/rehab-projects`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      if (!response.ok) throw new Error("backend unavailable");
  ...
  const deleteProject = async (projectId) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/rehab-projects/${projectId}`, { method: "DELETE" });
      if (!response.ok) throw new Error("backend unavailable");
  ...
  const toggleFavorite = async (project) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/rehab-projects/${project.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(updated) });
      if (!response.ok) throw new Error("backend unavailable");
      const saved = await response.json();
