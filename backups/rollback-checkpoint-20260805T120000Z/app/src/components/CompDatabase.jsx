...
export default function CompDatabase({
  ...
  const [providerStatus, setProviderStatus] = useState({ provider: "manual", status: "Manual Entry Ready", configured: true, keyPresent: false });
  ...
  useEffect(() => {
    const loadProviderStatus = async () => {
      try {
        ...
        setProviderStatus(status || { provider: "manual", status: "Manual Entry Ready", configured: true, keyPresent: false });
      } catch (error) {
  ...
  const handleProviderTest = async () => {
    try {
      ...
      setMessage({ type: "success", text: `Provider test: ${result.status}` });
    } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    const normalizedPayload = normalizeCompPayload({
      ...
      provider: providerStatus.provider || "manual",
      providerImported: providerStatus.provider !== "manual" && providerStatus.status !== "Provider Not Configured",
      manuallyEntered: providerStatus.provider === "manual",
      verified: false,
          ...
          <div style={styles.gridTwo}>
            <div style={styles.section}>
              ...
              <div style={styles.formActions}>
                ...
                <button type="button" style={styles.secondaryButton} onClick={handleProviderTest}>TEST PROVIDER</button>
              </div>
            </div>
