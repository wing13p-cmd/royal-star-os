...
export function buildRuntimeConfig(options = {}) {
  ...
  const requireAbsoluteApiBase = getEnvValue(env, 'RSOS_REQUIRE_ABSOLUTE_API_BASE') === 'true';
  ...
  if (options.requireProductionConfig && mode === 'production') {
    ...
    if (requireAbsoluteApiBase && !getEnvValue(env, 'RSOS_API_BASE_URL') && !getEnvValue(env, 'API_BASE_URL')) {
      missing.push('RSOS_API_BASE_URL or API_BASE_URL');
    }
    if (missing.length) {
