...
import { buildOfferIntelligence, normalizeDealForIntelligence, buildUnifiedUnderwritingIntelligence, buildUnderwritingMetrics, buildEnterpriseDealIntelligenceSummary } from "./intelligenceUpgradeEngine.js";
...
function formatCurrencyInput(value, entered, zeroLabel = "$0") {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return "Insufficient Data";
  if (parsed === 0 && !entered) return "Not Entered";
  if (parsed === 0 && entered) return zeroLabel;
...
function buildDealIntelligenceDisplayBindings({ deal, underwritingDecision, recommendation, financing }) {
...
function buildDealAnalysis(deal, comps, neighborhoods, lenders = [], selectedLenderId = "") {
  ...
  const flipRecommendationSignal = estimatedFlipProfit > 0 ? "Buy" : "Do Not Purchase";
  const brrrrRecommendationSignal = unifiedUnderwriting?.brrrrAnalysis?.monthlyCashFlow == null
    ? "Insufficient Data"
    : (unifiedUnderwriting.brrrrAnalysis.monthlyCashFlow > 0 && safeNumber(unifiedUnderwriting?.brrrrAnalysis?.debtServiceCoverageRatio) >= 1.2)
      ? "Buy"
      : unifiedUnderwriting.brrrrAnalysis.monthlyCashFlow > 0
        ? "Conditional Buy"
        : "Do Not Purchase";
  const missingFinancialInputs = Array.isArray(unifiedUnderwriting?.recommendation?.missingFinancialInputs)
    ? unifiedUnderwriting.recommendation.missingFinancialInputs
    : [];
  const decisionBlockers = [
    ...(Array.isArray(sharedDecision?.blockingActions) ? sharedDecision.blockingActions : []),
    ...(missingFinancialInputs.length > 0 ? [`Missing required financial inputs: ${missingFinancialInputs.join(", ")}`] : []),
  ];
  const recommendationReconciliation = reconcileRecommendations([
    { source: "Flip", recommendation: flipRecommendationSignal },
    { source: "BRRRR", recommendation: brrrrRecommendationSignal },
    { source: "AI Decision", recommendation: aiDecisionEngine?.dealDecision?.recommendedAction || sharedDecision?.aiDecision || "Insufficient Data" },
    { source: "Base Recommendation", recommendation: sharedDecision?.baseRecommendation || recommendation?.primaryRecommendation || "Insufficient Data" },
    { source: "Enterprise Route", recommendation: sharedDecision?.enterpriseRoute || "Insufficient Data" },
    { source: "Executive Decision", recommendation: sharedDecision?.executiveDecision || recommendation?.executiveSummary?.overallRecommendation || "Insufficient Data" },
    { source: "Downside Recommendation", recommendation: sharedDecision?.downsideRecommendation || "Insufficient Data" },
    { source: "Buy Box", recommendation: buyBoxResult.result },
  ], {
...
export default function DealIntelligence({ onBack, currentView = "dealIntelligence", onNavigate }) {
  ...
  const loadData = async () => {
    try {
      ...
      await refreshConnectionDetails(normalizedDeals.length);
    } catch (error) {
      ...
      if (typeof window !== "undefined") {
        try {
          ...
          await refreshConnectionDetails(Array.isArray(fallbackDeals) ? fallbackDeals.length : 0);
        } catch (localError) {
  ...
  const refreshConnectionDetails = async (recordCount = deals.length) => {
    const details = await fetchBackendConnectionStatus({
      primaryEndpoint: "/api/deals",
      fallbackStorageKeys: ["royalStarDeals", "royalStarComps", "royalStarNeighborhoods", "royalStarLenders"],
      recordCount,
    });
  ...
  useEffect(() => {
    ...
    window.addEventListener("royalStarPropertiesUpdated", refreshOnPropertyChange);
    ...
    return () => {
      ...
      window.removeEventListener("royalStarPropertiesUpdated", refreshOnPropertyChange);
  ...
  useEffect(() => {
    ...
    void refreshConnectionDetails(deals.length);
  }, [showConnectionDetails, deals.length]);
  ...
  const analysis = useMemo(
    () => {
      const computedDeals = displayDeals
        .map((deal, index) => {
          ...
          const backendRecord = Array.isArray(liveDealIntelligence)
            ...
            : null;
          buildDealIntelligenceViewModel({
            deal,
            backendRecord,
            fallback: {
              dealScore: 0,
      ...
      return [...computedDeals].sort((a, b) => b.dealScore - a.dealScore);
    },
    ...
    return {
      ...
      topDeal: topDeal
        ? {
            ...
            actualLoanAmountEntered: isEntered(topDeal.actualLoanAmount) || isEntered(topDeal.loanAmount),
            initialCashInvestedEntered: isEntered(topDeal.initialCashInvested) || isEntered(topDeal.cashRequired),
            monthlyCarryEntered: isEntered(topDeal.monthlyCarry) || isEntered(topDeal.monthlyPayment),
            ...
            decisionHierarchy: topDeal.decisionHierarchy || {
              ...
              baseRecommendation: safeDisplay(topDeal.baseRecommendation || topDeal.recommendationDecision, "Insufficient Data"),
              downsideRecommendation: safeDisplay(topDeal.sharedDecision?.downsideRecommendation || "Insufficient Data", "Insufficient Data"),
              confidence: safeNumber(topDeal.decisionConfidence ?? 0),
              conditions: Array.isArray(topDeal.sharedDecision?.blockingActions) ? topDeal.sharedDecision.blockingActions : [],
              controllingRule: safeDisplay(topDeal.recommendationReconciliation?.controllingRule, "Insufficient Data"),
            },
      ...
      <main style={styles.main}>
        ...
        <section style={styles.card}>
          <div style={styles.cardHeader}>
            ...
            <button type="button" style={styles.connectionBadge} onClick={() => setShowConnectionDetails(true)}>{connectionState}</button>
          </div>
          ...
          {analysis.length === 0 ? (
            ...
            <>
              <div style={styles.summaryGrid}>
                ...
                <SummaryCard label="Projected ARV" value={summary?.topDeal?.arvBreakdown?.projectedArv > 0 ? formatCurrency(summary.topDeal.arvBreakdown.projectedArv) : "Insufficient Data"} />
                ...
                <SummaryCard label={summary?.topDeal?.loanAmountDisplayLabel || "Actual Loan Amount"} value={formatCurrencyInput(summary?.topDeal?.actualLoanAmount || summary?.topDeal?.loanAmount || 0, summary?.topDeal?.actualLoanAmountEntered)} />
              ...
              <div style={styles.scenarioSection}>
                <div style={styles.explanationBox}>
                  ...
                  <div style={styles.listItem}>Recommendation Reconciliation: {summary?.topDeal?.recommendationReconciliation?.displayRecommendation || "Insufficient Data"}</div>
                  <div style={styles.listItem}>{summary?.topDeal?.recommendationReconciliation?.explanation || "Insufficient Data"}</div>
            ...
            <div style={styles.modalOverlay} onClick={() => setShowConnectionDetails(false)}>
              <div style={styles.modalCard} onClick={(event) => event.stopPropagation()}>
                <div style={styles.modalHeader}>
                  <strong>Backend Connection Details</strong>
                  <button type="button" style={styles.smallButton} onClick={() => setShowConnectionDetails(false)}>Close</button>
                </div>
                <div style={styles.modalBody}>
                  ...
                  <div>Deals API: /api/deals</div>
                  <div>Comps API: /api/comps</div>
                  <div>Neighborhoods API: /api/neighborhoods</div>
                  <div>Lenders API: /api/lenders</div>
                  <div>Fallback: {connectionDetails?.fallbackSource || "localStorage royalStarDeals/royalStarComps/royalStarNeighborhoods/royalStarLenders"}</div>
                  <div>Current loaded counts: deals {deals.length}, comps {comps.length}, neighborhoods {neighborhoods.length}, lenders {lenders.length}</div>
                </div>
                <div style={styles.actionRow}>
                  <button type="button" style={styles.smallButton} onClick={async () => {
                    await loadData();
                    await refreshConnectionDetails(deals.length);
                  }}>
                    Refresh Status
                  </button>
                </div>
