...
import { buildMaterialMatrixIntelligence } from "./optionExpansionIntelligence.js";
...
export default function MaterialMatrix({ onBack, onOpenDealAnalyzer, onOpenFlipAnalyzer, onOpenBrrrrAnalyzer, onOpenProductVault, onOpenContractorHub, onOpenCompDatabase, onOpenNeighborhoodDatabase, onOpenPortfolioDashboard, onOpenPropertyDatabase, onOpenVendorDatabase }) {
  ...
  const materialMatrixIntelligence = useMemo(() => {
    return buildMaterialMatrixIntelligence({ materials: normalizedMaterials, products, properties });
  }, [normalizedMaterials, products, properties]);
    ...
    <div style={styles.page}>
      ...
      <main style={styles.main}>
        ...
        <section style={styles.insightCard}>
          <div style={styles.kicker}>INTELLIGENCE</div>
          <div style={styles.insightTitle}>Budget posture is {materialMatrixIntelligence.summary.budgetHealth.toLowerCase()}.</div>
          <div style={styles.insightMeta}>Total budget {formatCurrency(materialMatrixIntelligence.summary.totalBudget)} with {materialMatrixIntelligence.summary.highPriorityCount} high-priority items.</div>
          <ul style={styles.insightList}>
            {materialMatrixIntelligence.recommendations.map((recommendation) => (
              ...
              <form onSubmit={handleSubmit} style={styles.formGrid}>
                ...
                <label style={styles.label}>Unit
                  <select name="unit" value={formValues.unit} onChange={handleInputChange} style={styles.input}>
                    {units.map((unit, index) => <option key={`${unit}-${index}`} value={unit}>{unit}</option>)}
                  </select>
                </label>
