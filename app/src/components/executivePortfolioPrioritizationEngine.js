...
function determinePriorityLevel(score, analysis = {}) {
...
function buildTimelineUrgencySignal(analysis = {}) {
...
export function buildExecutivePortfolioPrioritizationEngine(payload = {}) {
  ...
  const rankings = normalizedDeals.map((deal) => {
    ...
    const timelineUrgency = buildTimelineUrgencySignal(analysis);
    ...
    const priorityLevel = determinePriorityLevel(priorityScore, { ...analysis, opportunityScore, forecastConfidence, dealScore, overallRisk: riskScore, buyBoxResult: analysis.buyBoxResult });
