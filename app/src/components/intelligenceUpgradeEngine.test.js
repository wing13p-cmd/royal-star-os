...
test("normalizeUnderwritingInputs preserves true-zero values and keeps missing values null", () => {
  const normalized = normalizeUnderwritingInputs({
    purchasePrice: "0",
    rehabBudget: "0",
    estimatedArv: "",
  ...
  assert.equal(normalized.purchasePrice, 0);
  assert.equal(normalized.rehabBudget, 0);
  assert.equal(normalized.annualInsurance, 0);
  assert.equal(normalized.annualInterestRate, 0);
  assert.equal(normalized.actualLoanAmount, 0);
  assert.equal(normalized.arv, null);
  assert.equal(normalized.annualTaxes, null);
  assert.equal(normalized.holdingMonths, null);
  assert.equal(normalized.totalInitialCashInvested, null);
});
