...
export function buildPredictiveMarketIntelligence(deal = {}, neighborhoods = [], comps = []) {
  ...
  const classification = opportunityScore >= 85
    ? "Strong Buy"
    : opportunityScore >= 70
      ? "Buy"
      : opportunityScore >= 55
        ...
        : opportunityScore >= 40
          ? "Watch"
          : "Avoid";
  const explanation = opportunityScore >= 85
    ? "The market shows strong appreciation, stable liquidity, and supportive forward momentum for acquisition."
    : opportunityScore >= 70
      ? "The market has favorable appreciation and manageable risk, supporting a disciplined purchase."
      : opportunityScore >= 55
        ? "The market is acceptable but should be underwritten conservatively until more data is available."
        : opportunityScore >= 40
          ? "The market shows mixed momentum and elevated risk, so the deal should be monitored closely."
          : "The market has weak momentum and elevated downside risk, so the opportunity is unattractive.";
...
export function buildOpportunityDetectionEngine(deal = {}, arv = {}, market = {}, buyBox = {}, dealMetrics = {}, brrrrMetrics = {}, flipMetrics = {}, holdMetrics = {}) {
  ...
  const equityCreated = safeNumber(normalizedBrrrrMetrics.equityCreated ?? normalizedDealMetrics.equityCreated ?? normalizedDealMetrics.equityCreatedValue ?? 0);
  ...
  const classification = overallOpportunityScore >= 88
    ? "Strong Buy"
    : overallOpportunityScore >= 74
      ? "Buy"
      : overallOpportunityScore >= 58
        ? "Neutral"
        : overallOpportunityScore >= 40
          ? "Watch"
          : "Avoid";
...
export function buildExecutiveMarketSummaryEngine(deal = {}, arv = {}, market = {}, opportunity = {}, forecast = {}, recommendation = {}) {
  ...
  const overallMarketRating = marketStabilityScore >= 80 && appreciationTrend >= 4
    ? "Strong"
    : marketStabilityScore >= 60 && appreciationTrend >= 2
      ? "Healthy"
      : marketStabilityScore >= 40
        ? "Mixed"
        : "Weak";

  const opportunityRating = opportunityScore >= 80
    ? "Strong"
    : opportunityScore >= 60
      ? "Favorable"
      : opportunityScore >= 40
  ...
  const recommendationLabel = opportunityScore >= 85 && forecastConfidence >= 80
    ? "Strong Buy"
    : opportunityScore >= 70 && forecastConfidence >= 70
      ? "Buy"
      : opportunityScore >= 55 && forecastConfidence >= 60
        ? "Hold"
        : opportunityScore >= 40
          ? "Review"
          : "Pass";
...
export function buildBuyBoxIntelligence(deal = {}, neighborhoods = []) {
  ...
  const decision = !propertyTypeAllowed
    ? "Automatic Reject"
    : rulesFailed.length === 0 && conditionalRules.length === 0 && overallScore >= 70 && neighborhoodScore >= 45
      ? "Strong Pass"
      : rulesFailed.length <= 1 && overallScore >= 62
        ? "Pass"
        : overallScore >= 58
          ? "Conditional Pass"
          : rulesFailed.some((rule) => rule.includes("ZIP"))
            ? "Selective Area Review"
            : "Outside Buy Box";
...
function buildSharedDecisionResult(normalizedDeal = {}, financingAnalysis = {}, arvAnalysis = {}, buyBoxAnalysis = {}, flipAnalysis = {}, riskProfile = {}, recommendation = {}, executiveMarketSummary = {}, options = {}) {
  ...
  let worstCaseRecommendation;
  let downsideRecommendation;
  let enterpriseRoute;
  let qualification;
  const reasons = [];
  const warnings = [];
...
function buildGovernanceSignals(financingAnalysis = {}, buyBoxAnalysis = {}, sharedDecision = {}) {
...
export function buildUnifiedUnderwritingIntelligence(deal = {}, comps = [], neighborhoods = []) {
  const normalizedDeal = normalizeDealForIntelligence(deal);
  const acquisitionIntelligence = buildAcquisitionIntelligenceEngine({ deal: normalizedDeal });
  const projectExecutionIntelligence = buildProjectExecutionIntelligenceEngine({
    project: {
      id: safeString(normalizedDeal.rehabProjectId || normalizedDeal.projectId || ""),
      linkedDealId: safeString(normalizedDeal.id || ""),
      projectName: safeString(normalizedDeal.projectName || normalizedDeal.propertyAddress || ""),
      strategy: safeString(normalizedDeal.strategy || normalizedDeal.exitStrategy || "Flip"),
      projectStatus: safeString(normalizedDeal.projectStatus || normalizedDeal.rehabStatus || "Planning"),
      projectManager: safeString(normalizedDeal.projectManager || ""),
      projectedStartDate: safeString(normalizedDeal.projectedStartDate || ""),
      projectedCompletionDate: safeString(normalizedDeal.projectedCompletionDate || ""),
      actualCompletionDate: safeString(normalizedDeal.actualCompletionDate || ""),
      percentComplete: safeNumber(normalizedDeal.rehabPercentComplete || normalizedDeal.percentComplete || 0),
      originalRehabBudget: safeNumber(normalizedDeal.rehabBudget || normalizedDeal.originalRehabBudget || 0),
      currentRehabBudget: safeNumber(normalizedDeal.currentRehabBudget || normalizedDeal.rehabBudget || 0),
      approvedChangeOrders: safeNumber(normalizedDeal.approvedChangeOrders || 0),
      pendingChangeOrders: safeNumber(normalizedDeal.pendingChangeOrders || 0),
      committedCost: safeNumber(normalizedDeal.committedCost || 0),
      amountPaid: safeNumber(normalizedDeal.amountPaid || 0),
      squareFeet: safeNumber(normalizedDeal.squareFeet || 0),
    },
    approvedArv: safeNumber(normalizedDeal.estimatedArv || normalizedDeal.arv || normalizedDeal.projectedARV || 0),
    approvedFinancingCost: safeNumber(normalizedDeal.financingCosts || normalizedDeal.financingCost || 0),
    permitCosts: safeNumber(normalizedDeal.permitCosts || 0),
    taxes: safeNumber(normalizedDeal.taxes || 0),
    contingency: safeNumber(normalizedDeal.contingency || 0),
    utilityCost: safeNumber(normalizedDeal.utilityCost || 0),
    security: safeNumber(normalizedDeal.securityCost || normalizedDeal.security || 0),
    cleanup: safeNumber(normalizedDeal.cleanupCost || 0),
    roomConditions: Array.isArray(normalizedDeal.roomConditions) ? normalizedDeal.roomConditions : [],
    knownDeficiencies: Array.isArray(normalizedDeal.knownDeficiencies) ? normalizedDeal.knownDeficiencies : [],
    requiredPermits: Array.isArray(normalizedDeal.requiredPermits) ? normalizedDeal.requiredPermits : [],
    draws: Array.isArray(normalizedDeal.draws) ? normalizedDeal.draws : [],
    changeOrders: Array.isArray(normalizedDeal.changeOrders) ? normalizedDeal.changeOrders : [],
    permitRecords: Array.isArray(normalizedDeal.inspections) ? normalizedDeal.inspections : [],
    photos: Array.isArray(normalizedDeal.projectPhotos) ? normalizedDeal.projectPhotos : [],
    documents: Array.isArray(normalizedDeal.projectDocuments) ? normalizedDeal.projectDocuments : [],
    qualityItems: Array.isArray(normalizedDeal.punchListItems) ? normalizedDeal.punchListItems : [],
    purchaseOrders: Array.isArray(normalizedDeal.purchaseOrders) ? normalizedDeal.purchaseOrders : [],
    bids: Array.isArray(normalizedDeal.contractorBids) ? normalizedDeal.contractorBids : [],
    contractor: normalizedDeal.contractorProfile || {
      id: safeString(normalizedDeal.contractorId || ""),
      companyName: safeString(normalizedDeal.contractorName || ""),
      trade: safeString(normalizedDeal.contractorTrade || ""),
      approvalStatus: safeString(normalizedDeal.contractorApprovalStatus || "Prospect"),
    },
    vendorPerformance: normalizedDeal.vendorPerformance || {},
    contractorPerformance: normalizedDeal.contractorPerformance || {},
    contract: normalizedDeal.contract || {},
    closeout: normalizedDeal.closeout || {},
    materialChange: normalizedDeal.materialChange || {},
  });
  ...
  const propertyManagementPercentNormalized = normalizePercent(deal.propertyManagementPercent ?? 8, { min: 0, max: 1 });
  const refinanceLtvNormalized = normalizePercent(deal.refinanceLtvPercent ?? deal.refinanceLtv ?? 75, { min: 0, max: 1 });
  const refinanceRateNormalized = normalizePercent(deal.refinanceInterestRate ?? normalizedInputs.annualInterestRate, { min: 0, max: 1 });

  const refinanceLtv = refinanceLtvNormalized.status === "ok" ? refinanceLtvNormalized.value : null;
  const refinanceRate = refinanceRateNormalized.status === "ok" ? refinanceRateNormalized.value : null;
  const maxLoanBasedOnLtv = arvAnalysis.supportedBaseArv > 0 && refinanceLtv !== null ? arvAnalysis.supportedBaseArv * refinanceLtv : null;
  const lenderApprovedLoan = normalizedInputs.lenderLoanAmount != null && normalizedInputs.lenderLoanAmount > 0
    ? normalizedInputs.lenderLoanAmount
    : null;

  const refinanceLoanConstraints = [
    maxLoanBasedOnLtv,
    lenderApprovedLoan,
  ].filter((value) => Number.isFinite(value) && value > 0);

  const refinanceLoanAmount = refinanceLoanConstraints.length > 0 ? Math.min(...refinanceLoanConstraints) : null;
  ...
  const sellingCostPercentNormalized = normalizePercent(normalizedInputs.sellingCostPercent ?? deal.sellingCostPercent ?? deal.sellingCostsPercent ?? 8, { min: 0, max: 1 });
  const sellingCostPercent = sellingCostPercentNormalized.status === "ok" ? sellingCostPercentNormalized.value : 0.08;
...
const holdingCosts = totalInterest + (annualPropertyTaxesValue ? annualPropertyTaxesValue * holdingMonths / 12 : 0) + (annualInsuranceValue ? annualInsuranceValue * holdingMonths / 12 : 0);
  ...
  const refinanceTermYearsValue = toOptionalNumber(deal.refinanceLoanTermYears);
  const refinanceTermMonthsValue = refinanceTermYearsValue && refinanceTermYearsValue > 0
    ? refinanceTermYearsValue * 12
    : (loanTermMonths > 0 ? loanTermMonths : null);

  let monthlyDebtService = null;
  if (refinanceLoanAmount != null && refinanceRate != null && refinanceTermMonthsValue && refinanceTermMonthsValue > 0) {
    const monthlyRefinanceRate = refinanceRate / 12;
    if (monthlyRefinanceRate <= 0) {
      monthlyDebtService = refinanceLoanAmount / refinanceTermMonthsValue;
    } else {
      const factor = Math.pow(1 + monthlyRefinanceRate, refinanceTermMonthsValue);
      monthlyDebtService = (refinanceLoanAmount * monthlyRefinanceRate * factor) / (factor - 1);
    }
  }

  const monthlyCarry = monthlyPayment;
  const incomeInputsKnown = monthlyRentValue != null && otherMonthlyIncomeValue != null;
  const expenseInputsKnown = annualPropertyTaxesValue != null
    && annualInsuranceValue != null
    && monthlyHoaValue != null
    && monthlyUtilitiesValue != null
    && otherMonthlyExpensesValue != null
    && vacancyPercentNormalized.status === "ok"
    && maintenancePercentNormalized.status === "ok"
    && capexPercentNormalized.status === "ok"
    && propertyManagementPercentNormalized.status === "ok";

  const monthlyGrossIncome = incomeInputsKnown ? monthlyRentValue + otherMonthlyIncomeValue : null;
  const vacancyExpense = incomeInputsKnown && vacancyPercentNormalized.status === "ok" ? monthlyGrossIncome * vacancyPercentNormalized.value : null;
  const maintenanceExpense = incomeInputsKnown && maintenancePercentNormalized.status === "ok" ? monthlyGrossIncome * maintenancePercentNormalized.value : null;
  const capexExpense = incomeInputsKnown && capexPercentNormalized.status === "ok" ? monthlyGrossIncome * capexPercentNormalized.value : null;
  const propertyManagementExpense = incomeInputsKnown && propertyManagementPercentNormalized.status === "ok" ? monthlyGrossIncome * propertyManagementPercentNormalized.value : null;

  const operatingExpenses = expenseInputsKnown && vacancyExpense != null && maintenanceExpense != null && capexExpense != null && propertyManagementExpense != null
    ? (annualPropertyTaxesValue / 12)
      + (annualInsuranceValue / 12)
      + monthlyHoaValue
      + monthlyUtilitiesValue
      + otherMonthlyExpensesValue
      + vacancyExpense
      + maintenanceExpense
      + capexExpense
  ...
  const nopI = monthlyGrossIncome != null && operatingExpenses != null ? monthlyGrossIncome - operatingExpenses : null;
  ...
  const missingCoreFinancialInputs = [];
  if (purchasePrice <= 0) missingCoreFinancialInputs.push("Purchase price");
  if (rehabBudget <= 0) missingCoreFinancialInputs.push("Rehab budget");
  if (arvAnalysis.supportedBaseArv <= 0) missingCoreFinancialInputs.push("ARV support");

  if (dealExitStrategy === "brrrr") {
    if (refinanceLtvNormalized.status !== "ok") missingCoreFinancialInputs.push("Refinance LTV");
  ...
  const decisionRecommendation = hasBlockingFinancialInputs
    ? "CONDITIONAL BUY"
    : projectedProfit > 0 && (arvAnalysis.supportedBaseArv > 0 || normalizedInputs.hasMissingData === false)
      ? "BUY"
      : (projectedProfit > 0 ? "CONDITIONAL BUY" : "REJECT");
    ...
    action: hasBlockingFinancialInputs
      ...
      : projectDecisionRecommendation === "CONTINUE PROJECT"
        ...
        : projectDecisionRecommendation === "CONTINUE REHAB"
          ? "CONTINUE REHAB"
          : projectDecisionRecommendation === "HOLD"
            ? "HOLD"
            : decisionRecommendation === "BUY"
              ? "PROCEED"
              : decisionRecommendation === "CONDITIONAL BUY"
                ? "REQUEST MORE DATA"
                : "REJECT",
  ...
  const investmentDecision = buildInvestmentDecisionEngine(normalizedDeal, {
    ...
    qualificationStatus: hasBlockingFinancialInputs ? "Insufficient Data" : "Qualified",
    warnings: [
      ...(rehabBudget <= 0 ? ["Rehab budget is missing"] : []),
      ...(arvAnalysis.supportedBaseArv <= 0 ? ["ARV support is missing"] : []),
      ...missingCoreFinancialInputs.map((field) => `${field} is missing or invalid`),
    ],
