...
import { createValuationOfferBuyBoxService } from "./valuationOfferBuyBoxService.js";
...
const backupDir = process.env.RSOS_BACKUP_DIR
  ? path.resolve(process.env.RSOS_BACKUP_DIR)
  : path.join(projectRootDir, "backups");
const bindHost = process.env.RSOS_BIND_HOST || "127.0.0.1";
const publicOrigin = String(process.env.RSOS_PUBLIC_ORIGIN || "").trim();
...
const dealPortfolioSyncAuditFile = path.join(dataDir, "deal-portfolio-sync-audit.json");
const underwritingAuditFile = path.join(dataDir, "underwriting-audit.json");
const enterpriseAuditFile = path.join(dataDir, "enterprise-audit.json");
const enterpriseReportsFile = path.join(dataDir, "enterprise-reports.json");
const enterpriseDocumentsFile = path.join(dataDir, "enterprise-documents.json");
const enterpriseKnowledgeFile = path.join(dataDir, "enterprise-knowledge.json");
const workflowTransitionsFile = path.join(dataDir, "workflow-transitions.json");
const diagnosticsHistoryFile = path.join(dataDir, "diagnostics-history.json");
...
const localDevOrigins = productionMode
  ? []
  : [
    "http://localhost:5173",
    "http://127.0.0.1:5173",
...
const compProviderAdapters = {
  ...
  attom: new AttomCompAdapter(compProviderConfig),
  "reso-mls": new ResoMlsCompAdapter(compProviderConfig),
  bridge: new BridgeResoAdapter(compProviderConfig),
  "county-import": new CountyImportAdapter(compProviderConfig),
};
const activeProviderKey = compProviderConfig.provider === "rentcast"
  ? "rentcast"
  : compProviderConfig.provider === "attom"
    ? "attom"
    : compProviderConfig.provider === "reso-mls"
      ? "reso-mls"
      : compProviderConfig.provider === "bridge"
        ? "bridge"
        : compProviderConfig.provider === "county-import"
          ? "county-import"
          : "manual";
...
async function ensureDirectoryWritable(directoryPath, label) {
  ...
  const probeFile = path.join(directoryPath, `.rsos-write-probe-${Date.now()}.tmp`);
...
async function acquireSingleWriterLock() {
  const lockFilePath = path.join(dataDir, ".rsos-writer.lock");
  const isPidRunning = (pidValue) => {
  ...
  const clearStaleLockIfNeeded = async () => {
    try {
      const existingPid = String(await fs.readFile(lockFilePath, "utf8")).trim();
      if (!isPidRunning(existingPid)) {
        await fs.rm(lockFilePath, { force: true });
      }
    } catch {
      // If lock file cannot be read, keep default lock-open behavior below.
    }
  };
...
async function validateStartupEnvironment() {
  ...
  await ensureDirectoryWritable(backupDir, "backup directory");
  await acquireSingleWriterLock();
}

await validateStartupEnvironment();
const compOperationsService = createCompOperationsService({ storageFilePath: path.join(dataDir, "comp-operations.json") });
await compOperationsService.initialize();
...
function mergeDealLinkageFields(existingDeal = {}, normalizedDeal = {}) {
  const merged = { ...normalizedDeal };
  const preserveIfBlank = ["linkedPropertyId", "propertyId", "parcelNumber", "mapUrl"];

  preserveIfBlank.forEach((field) => {
    const nextValue = getStringValue(merged[field]);
    const existingValue = getStringValue(existingDeal[field]);
    if (!nextValue && existingValue) {
      merged[field] = existingValue;
    }
  });

  if (!getStringValue(merged.linkedPropertyId) && getStringValue(merged.propertyId)) {
    merged.linkedPropertyId = merged.propertyId;
  }
  if (!getStringValue(merged.propertyId) && getStringValue(merged.linkedPropertyId)) {
    merged.propertyId = merged.linkedPropertyId;
  }
...
function normalizePropertyPayload(payload = {}) {
  return {
    ...
    parcelRetrievalSourceUrl: getStringValue(payload.parcelRetrievalSourceUrl),
    parcelRetrievedAt: getStringValue(payload.parcelRetrievedAt),
    parcelVerificationStatus: getStringValue(payload.parcelVerificationStatus, "Pending"),
    parcelUserConfirmedAt: getStringValue(payload.parcelUserConfirmedAt),
    ...
    dataQualityHistory: getStringValue(payload.dataQualityHistory, "[]"),
    workflowTransitionHistory: getStringValue(payload.workflowTransitionHistory, "[]"),
    workflowBlockers: getStringValue(payload.workflowBlockers, "[]"),
    workflowCompletionPercent: getNumberValue(payload.workflowCompletionPercent),
    workflowRollbackReason: getStringValue(payload.workflowRollbackReason),
...
function normalizeDealPayload(payload = {}) {
  return {
    ...
    notes: getStringValue(payload.notes),
    linkedPropertyId: getStringValue(payload.linkedPropertyId ?? payload.propertyId),
    propertyId: getStringValue(payload.propertyId ?? payload.linkedPropertyId),
    parcelNumber: getStringValue(payload.parcelNumber),
    mapUrl: getStringValue(payload.mapUrl),
    pipelineStage: getStringValue(payload.pipelineStage, "New Lead"),
    workflowTransitionHistory: Array.isArray(payload.workflowTransitionHistory)
      ? payload.workflowTransitionHistory
      : (typeof payload.workflowTransitionHistory === "string"
        ? (() => {
          try {
            const parsed = JSON.parse(payload.workflowTransitionHistory);
...
async function getLatestTimestampFromDirectory(directoryPath, matcher) {
  try {
    ...
    for (const name of matched) {
      try {
        ...
        // Ignore a single unreadable file and keep scanning.
      }
    }
...
async function getLatestBackupTimestamp() {
  const recoveryDir = path.join(dataDir, ".recovery");
  const projectBackupsDir = backupDir;

  const [recoveryBackupTimestamp, projectBackupTimestamp] = await Promise.all([
    getLatestTimestampFromDirectory(recoveryDir, (name) => name.endsWith(".bak")),
    getLatestTimestampFromDirectory(projectBackupsDir, (name) => /royal-star-os-backup-.*\.json$/i.test(name)),
  ]);

  const timestamps = [recoveryBackupTimestamp, projectBackupTimestamp]
    .map((value) => Date.parse(value || ""))
    .filter((value) => Number.isFinite(value));
  if (!timestamps.length) return "Insufficient Data";
...
async function buildRuntimeStatusPayload() {
  ...
  const persistenceReady = Object.values(dataFiles).every(Boolean);
  const diagnosticsHistory = await readDiagnosticsHistoryFile().catch(() => []);
  const latestDiagnostic = Array.isArray(diagnosticsHistory)
    ? [...diagnosticsHistory].sort((left, right) => String(right?.timestamp || "").localeCompare(String(left?.timestamp || "")))[0]
    : null;
  const lastBackupTimestamp = await getLatestBackupTimestamp();
  ...
  return {
    ...
    rsosOwnership: "RSOS-owned",
    backendPort: port,
    apiHealthStatus: "ok",
    listenerCount: 1,
    responseTimeMs: 0,
    lastSuccessfulResponse: now,
    persistenceState: persistenceReady ? "Ready" : "Degraded",
    persistenceReady,
    dataDirectoryState: "Ready",
    dataDirectoryReady: true,
    dataFiles,
    lastVerificationResult: latestDiagnostic
      ? `${String(latestDiagnostic.status || "unknown")} (${String(latestDiagnostic.timestamp || "Unknown")})`
      : "Insufficient Data",
    lastVerificationTimestamp: latestDiagnostic?.timestamp || "Insufficient Data",
    lastBackupTimestamp,
  };
}
...
function buildCompProviderStatus() {
  ...
  return {
    ...
    availableProviders: Object.keys(compProviderAdapters),
    providerStatuses: Object.fromEntries(Object.entries(compProviderAdapters).map(([name, adapter]) => [name, adapter.getProviderStatus()])),
  };
}
...
async function writeDealPortfolioSyncAuditFile(events) { await writeJsonFile(dealPortfolioSyncAuditFile, events); }
async function readDealPortfolioSyncAuditFile() { return readJsonArrayFile(dealPortfolioSyncAuditFile, [], "deal-portfolio-sync-audit"); }
async function writeUnderwritingAuditFile(events) { await writeJsonFile(underwritingAuditFile, events); }
async function readUnderwritingAuditFile() { return readJsonArrayFile(underwritingAuditFile, [], "underwriting-audit"); }
async function writeEnterpriseAuditFile(events) { await writeJsonFile(enterpriseAuditFile, events); }
async function readEnterpriseAuditFile() { return readJsonArrayFile(enterpriseAuditFile, [], "enterprise-audit"); }
async function writeEnterpriseReportsFile(items) { await writeJsonFile(enterpriseReportsFile, items); }
async function readEnterpriseReportsFile() { return readJsonArrayFile(enterpriseReportsFile, [], "enterprise-reports"); }
async function writeEnterpriseDocumentsFile(items) { await writeJsonFile(enterpriseDocumentsFile, items); }
async function readEnterpriseDocumentsFile() { return readJsonArrayFile(enterpriseDocumentsFile, [], "enterprise-documents"); }
async function writeEnterpriseKnowledgeFile(items) { await writeJsonFile(enterpriseKnowledgeFile, items); }
async function readEnterpriseKnowledgeFile() { return readJsonArrayFile(enterpriseKnowledgeFile, [], "enterprise-knowledge"); }
async function writeWorkflowTransitionsFile(items) { await writeJsonFile(workflowTransitionsFile, items); }
async function readWorkflowTransitionsFile() { return readJsonArrayFile(workflowTransitionsFile, [], "workflow-transitions"); }
async function writeDiagnosticsHistoryFile(items) { await writeJsonFile(diagnosticsHistoryFile, items); }
async function readDiagnosticsHistoryFile() { return readJsonArrayFile(diagnosticsHistoryFile, [], "diagnostics-history"); }
...
const dealPortfolioSyncService = createDealPortfolioSyncService({
  readDeals: readDealsFile,
  writeDeals: writeDealsFile,
  readPortfolio: readPortfolioFile,
  writePortfolio: writePortfolioFile,
  readAudit: readDealPortfolioSyncAuditFile,
  writeAudit: writeDealPortfolioSyncAuditFile,
});

const crossModuleSyncService = createCrossModuleSyncService({
  readDeals: readDealsFile,
  writeDeals: writeDealsFile,
  readProperties: readPropertiesFile,
  writeProperties: writePropertiesFile,
  readPortfolio: readPortfolioFile,
  writePortfolio: writePortfolioFile,
  readRehabProjects: readRehabProjectsFile,
  writeRehabProjects: writeRehabProjectsFile,
});

const dealPropertySyncService = createDealPropertySyncService({
  readDeals: readDealsFile,
  writeDeals: writeDealsFile,
  readProperties: readPropertiesFile,
  writeProperties: writePropertiesFile,
...
const valuationOfferBuyBoxService = createValuationOfferBuyBoxService({
  readDeals: readDealsFile,
  writeDeals: writeDealsFile,
  readComps: readCompsFile,
  readNeighborhoods: readNeighborhoodsFile,
  readAudit: readUnderwritingAuditFile,
  writeAudit: writeUnderwritingAuditFile,
});

const enterpriseAutomationLayerService = createEnterpriseAutomationLayerService({
  readDeals: readDealsFile,
  writeDeals: writeDealsFile,
  readProperties: readPropertiesFile,
  writeProperties: writePropertiesFile,
  ...
  readComps: readCompsFile,
  readNeighborhoods: readNeighborhoodsFile,
  readContractors: readContractorsFile,
  readVendors: readVendorsFile,
  readLenders: readLendersFile,
  readProducts: readProductsFile,
  readMaterials: readMaterialsFile,
  readRehabProjects: readRehabProjectsFile,
  writeRehabProjects: writeRehabProjectsFile,
  readAppraisalPackets: readAppraisalPacketsFile,
  writeAppraisalPackets: writeAppraisalPacketsFile,
  readDealIntelligence: readDealIntelligenceFile,
  readUnderwritingAudit: readUnderwritingAuditFile,
  readSyncAudit: readDealPortfolioSyncAuditFile,
  readEnterpriseAudit: readEnterpriseAuditFile,
  writeEnterpriseAudit: writeEnterpriseAuditFile,
  readReports: readEnterpriseReportsFile,
  writeReports: writeEnterpriseReportsFile,
  readDocuments: readEnterpriseDocumentsFile,
  writeDocuments: writeEnterpriseDocumentsFile,
  readKnowledge: readEnterpriseKnowledgeFile,
  writeKnowledge: writeEnterpriseKnowledgeFile,
  readWorkflowTransitions: readWorkflowTransitionsFile,
  writeWorkflowTransitions: writeWorkflowTransitionsFile,
  readDiagnosticsHistory: readDiagnosticsHistoryFile,
  writeDiagnosticsHistory: writeDiagnosticsHistoryFile,
});
...
function isOperatorAuthorized(req) {
  ...
  if (context.isLocalHost || context.isAdmin) return true;
  const operatorToken = String(process.env.RSOS_OPERATOR_TOKEN || "").trim();
...
async function handleCollection(req, res, entity, readFile, writeFile, normalizeFn, validateFn, newIdPrefix) {
  ...
  if (req.method === "POST" && !itemId) {
    ...
    if (entity === "deal" || entity === "property") {
      try {
        await dealPropertySyncService.synchronizeAfterSave({
          sourceEntity: entity,
          savedRecordId: item.id,
          sourceModule: entity === "deal" ? "Deal Intake" : "Property Database",
        });
      } catch (error) {
        console.error(`[RSOS] ${entity} synchronization failed after create`, error);
      }
    }
    if (entity === "deal") {
      try {
  ...
  if (req.method === "PUT" && itemId) {
    ...
    if (entity === "deal" || entity === "property") {
      try {
        await dealPropertySyncService.synchronizeAfterSave({
          sourceEntity: entity,
          savedRecordId: updatedItem.id,
          sourceModule: entity === "deal" ? "Deal Intake" : "Property Database",
        });
      } catch (error) {
        console.error(`[RSOS] ${entity} synchronization failed after update`, error);
      }
    }
    if (entity === "deal") {
      try {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === "/operator/health" || pathname === "/api/operator/health") {
      if (!isOperatorAuthorized(req)) {
        sendStructuredError(res, 403, "Operator authentication required", "permission_error", req.requestContext?.requestId || "");
        return;
      }

      const runtimeStatus = await buildRuntimeStatusPayload();
      const frontendBuildDir = path.join(projectRootDir, "app", "dist");
      const swFile = path.join(frontendBuildDir, "sw.js");
      const manifestFile = path.join(frontendBuildDir, "manifest.webmanifest");
      const iconFile = path.join(frontendBuildDir, "icons", "icon-192.png");

      const checks = {
        backendStatus: runtimeStatus.backendStatus,
        persistenceReady: runtimeStatus.persistenceReady,
        writableDataDirectory: true,
        frontendBuildAvailable: false,
      ...
      try {
        ...
        checks.writableDataDirectory = false;
      }

      try {
        await fs.access(frontendBuildDir, fsConstants.R_OK);
        checks.frontendBuildAvailable = true;
      } catch {
        checks.frontendBuildAvailable = false;
      }

      try {
        await fs.access(swFile, fsConstants.R_OK);
        checks.serviceWorkerAvailable = true;
      } catch {
        checks.serviceWorkerAvailable = false;
      }

      try {
        await fs.access(manifestFile, fsConstants.R_OK);
        checks.manifestAvailable = true;
      } catch {
        checks.manifestAvailable = false;
      }

      try {
        await fs.access(iconFile, fsConstants.R_OK);
        checks.iconAvailable = true;
      } catch {
        checks.iconAvailable = false;
      }

      try {
        await fs.access(backupDir, fsConstants.R_OK | fsConstants.W_OK);
      } catch {
        checks.backupDirectoryReady = false;
      }
    ...
    if (pathname === "/api/provider-onboarding") {
      if (req.method === "GET") {
        const provider = url.searchParams.get("provider") || "manual";
        const state = providerOnboardingService.getProviderState(provider);
        const masked = providerOnboardingService.getMaskedCredentialStatus(provider);
        sendJson(res, 200, {
          provider,
          status: state.connectionStatus,
          active: state.active,
          maskedCredentialStatus: masked,
          diagnostics: providerOnboardingService.getDiagnostics(provider),
        });
        return;
      }
      if (req.method === "POST") {
        ...
        if (payload.action === "save") {
          providerOnboardingService.saveCredential(provider, payload);
          sendJson(res, 200, { ok: true, provider, status: "Saved" });
          return;
        }
        if (payload.action === "test") {
          sendJson(res, 200, providerOnboardingService.testConnection(provider));
          return;
        }
        if (payload.action === "activate") {
          sendJson(res, 200, providerOnboardingService.activateProvider(provider, payload.admin || "System Administrator"));
          return;
        }
        if (payload.action === "deactivate") {
          sendJson(res, 200, providerOnboardingService.deactivateProvider(provider));
          return;
        }
        if (payload.action === "rotate") {
          sendJson(res, 200, providerOnboardingService.rotateCredential(provider, payload));
          return;
        }
        if (payload.action === "remove") {
          sendJson(res, 200, providerOnboardingService.removeCredential(provider));
          return;
        }
        sendJson(res, 200, { ok: true, provider, status: "No-op" });
        return;
      }
      sendJson(res, 405, { error: "Method not allowed" });
      return;
    ...
    if (pathname === "/api/providers/enterprise-platform") {
      if (req.method !== "GET") {
        sendJson(res, 405, { error: "Method not allowed" });
        return;
      }

      const diagnosticsActor = url.searchParams.get("actor") || "UNKNOWN";
      const adminDiagnostics = enterpriseProviderPlatform.controlLayer.adminHealth.snapshot(diagnosticsActor);

      sendJson(res, 200, {
        advisoryOnly: true,
        liveRequestsAllowed: false,
        providerRegistry: enterpriseProviderPlatform.registry,
        diagnostics: enterpriseProviderPlatform.diagnostics(),
        providerStatusDashboard: enterpriseProviderPlatform.providerStatusDashboard(),
        reviewQueueSummary: enterpriseProviderPlatform.reviewQueue.summary(),
        enterpriseControlLayer: {
          providerMonitor: enterpriseProviderPlatform.controlLayer.monitor.list(),
          syncOperations: enterpriseProviderPlatform.controlLayer.sync.list(),
          scheduledJobs: enterpriseProviderPlatform.controlLayer.scheduler.list(),
          dataQualityTemplate: enterpriseProviderPlatform.controlLayer.documentationTemplate,
          reviewQueue: enterpriseProviderPlatform.controlLayer.reviewQueue.summary(),
          rateLimitUsage: enterpriseProviderPlatform.controlLayer.usageMonitor.list(),
          outageState: enterpriseProviderPlatform.controlLayer.outage.listOutages(),
          cacheState: enterpriseProviderPlatform.controlLayer.cache.list(),
          auditSummary: {
            totalRecords: enterpriseProviderPlatform.controlLayer.audit.list().length,
          },
          adminDiagnostics,
        },
        syncState: enterpriseProviderPlatform.syncEngine.getState(),
        auditSummary: enterpriseProviderPlatform.audit.summary(),
      });
      return;
    }
    ...
    if (pathname === "/api/runtime-status") {
      const payload = await buildRuntimeStatusPayload();
    ...
    if (segments[0] === "api" && segments[1] === "cross-module-sync") {
      if (req.method === "GET") {
        const state = await crossModuleSyncService.previewSynchronizedState();
      ...
      if (req.method === "POST") {
        ...
        const actor = getStringValue(payload.actor, "System Administrator");
        const dealId = getStringValue(payload.dealId);
        const result = dealId
          ? await crossModuleSyncService.synchronizeDealImport(dealId, actor)
          : await crossModuleSyncService.synchronizeAll(actor);
        sendJson(res, 200, result);
        return;
      }

      sendJson(res, 404, { error: "Cross-module sync endpoint not found" });
      return;
    }
    ...
    if (segments[0] === "api" && segments[1] === "underwriting") {
      if (req.method === "GET") {
        const auth = authorizeRequest(req, res, "read");
        if (!auth.allowed) return;

        if (segments[2] === "audit") {
          const dealId = getStringValue(url.searchParams.get("dealId") || "");
          const auditEvents = await valuationOfferBuyBoxService.listAuditEvents(dealId);
          sendJson(res, 200, { auditEvents });
          return;
        }

        if (segments[2] === "valuation" && segments[3] === "history") {
          const dealId = getStringValue(url.searchParams.get("dealId") || "");
          const history = await valuationOfferBuyBoxService.getArvAuditHistory(dealId);
          sendJson(res, 200, { history });
          return;
        }

        if (segments[2] === "offer" && segments[3] === "history") {
          const dealId = getStringValue(url.searchParams.get("dealId") || "");
          const history = await valuationOfferBuyBoxService.getOfferAuditHistory(dealId);
          sendJson(res, 200, { history });
          return;
        }
        ...
        if (segments[2] === "snapshot") {
          const dealId = getStringValue(url.searchParams.get("dealId") || "");
          const snapshot = await valuationOfferBuyBoxService.getUnifiedIntelligenceSnapshot(dealId);
          sendJson(res, 200, snapshot);
          return;
        }

        if (segments[2] === "appraiser-packet-support") {
          const dealId = getStringValue(url.searchParams.get("dealId") || "");
          const support = await valuationOfferBuyBoxService.getAppraiserPacketSupport(dealId);
      ...
      if (req.method === "POST") {
        ...
        if (segments[2] === "valuation" && segments[3] === "preview") {
          const preview = await valuationOfferBuyBoxService.previewArvRecommendation(dealId);
          sendJson(res, 200, preview);
          return;
        }

        if (segments[2] === "valuation" && segments[3] === "approve") {
          const approved = await valuationOfferBuyBoxService.approveArvRecommendation(dealId, payload.selectedValue, payload.userApproval === true, actor);
          await syncDealIntelligenceStore();
          sendJson(res, 200, approved);
          return;
        }

        if (segments[2] === "offer" && segments[3] === "preview") {
          const preview = await valuationOfferBuyBoxService.previewOfferRecommendation(dealId, payload.assumptions || {});
          sendJson(res, 200, preview);
          return;
        }

        if (segments[2] === "offer" && segments[3] === "approve") {
          const approved = await valuationOfferBuyBoxService.approveOfferGuidance(dealId, payload.userApproval === true, actor);
          await syncDealIntelligenceStore();
          sendJson(res, 200, approved);
          return;
        }

        if (segments[2] === "buy-box" && segments[3] === "preview") {
          const preview = await valuationOfferBuyBoxService.previewBuyBoxEvaluation(dealId);
          sendJson(res, 200, preview);
          return;
        }

        if (segments[2] === "buy-box" && segments[3] === "approve") {
          const approved = await valuationOfferBuyBoxService.approveBuyBoxReview(dealId, payload.userApproval === true, actor);
          await syncDealIntelligenceStore();
          sendJson(res, 200, approved);
          return;
        }

        if (segments[2] === "rollback") {
          const rollback = await valuationOfferBuyBoxService.rollbackLatestApproval(dealId, actor);
          await syncDealIntelligenceStore();
          sendJson(res, 200, rollback);
          return;
        }
      }

      sendJson(res, 404, { error: "Underwriting endpoint not found" });
      return;
    }

    if (segments[0] === "api" && segments[1] === "enterprise") {
      if (req.method === "GET") {
        const auth = authorizeRequest(req, res, "read");
        if (!auth.allowed) return;

        if (segments[2] === "command-center" && (segments[3] === "snapshot" || !segments[3])) {
          const snapshot = await enterpriseAutomationLayerService.buildExecutiveSnapshot();
          sendJson(res, 200, snapshot);
          return;
        }

        if (segments[2] === "command-center" && segments[3] === "audit") {
          const history = await enterpriseAutomationLayerService.getExecutiveAuditHistory();
          sendJson(res, 200, { history });
          return;
        }

        if (segments[2] === "search" && segments[3] === "refresh") {
          const result = await enterpriseAutomationLayerService.refreshSearchIndex();
          sendJson(res, 200, result);
          return;
        }

        if (segments[2] === "search") {
          const query = getStringValue(url.searchParams.get("q") || "");
          const entityType = getStringValue(url.searchParams.get("entityType") || "");
          const results = await enterpriseAutomationLayerService.searchAllEntities(query, { entityType });
          sendJson(res, 200, results);
          return;
        }

        if (segments[2] === "reports" && segments[3] === "audit") {
          const history = await enterpriseAutomationLayerService.getReportAuditHistory();
          sendJson(res, 200, { history });
          return;
        }

        if (segments[2] === "documents" && segments[3] === "audit") {
          const documentId = getStringValue(url.searchParams.get("documentId") || "");
          const history = await enterpriseAutomationLayerService.getDocumentAuditHistory(documentId);
          sendJson(res, 200, { history });
          return;
        }

        if (segments[2] === "knowledge" && segments[3] === "audit") {
          const history = await enterpriseAutomationLayerService.getKnowledgeAuditHistory();
          sendJson(res, 200, { history });
          return;
        }

        if (segments[2] === "knowledge" && segments[3] === "search") {
          const query = getStringValue(url.searchParams.get("q") || "");
          const category = getStringValue(url.searchParams.get("category") || "");
          const results = await enterpriseAutomationLayerService.searchKnowledge(query, { category });
          sendJson(res, 200, results);
          return;
        }

        if (segments[2] === "workflow" && segments[3] === "status") {
          const type = getStringValue(url.searchParams.get("type") || "");
          const sourceId = getStringValue(url.searchParams.get("sourceId") || "");
          const status = await enterpriseAutomationLayerService.getWorkflowTransitionStatus(type, sourceId);
          sendJson(res, 200, status);
          return;
        }

        if (segments[2] === "diagnostics" && segments[3] === "history") {
          const history = await enterpriseAutomationLayerService.getDiagnosticsHistory();
          sendJson(res, 200, { history });
          return;
        }

        if (segments[2] === "forecast" && segments[3] === "audit") {
          const history = await enterpriseAutomationLayerService.getForecastAuditHistory();
          sendJson(res, 200, { history });
          return;
        }
      }
      ...
      if (req.method === "POST") {
        ...
        if (segments[2] === "reports" && segments[3] === "generate") {
          const result = await enterpriseAutomationLayerService.generateReport(getStringValue(payload.reportType), {
            dealId: getStringValue(payload.dealId),
            format: getStringValue(payload.format),
            actor: getStringValue(payload.actor, "System Administrator"),
          });
          sendJson(res, 200, result);
          return;
        }

        if (segments[2] === "documents" && segments[3] === "generate") {
          const result = await enterpriseAutomationLayerService.generateDocumentDraft(
            getStringValue(payload.type),
            getStringValue(payload.recordId),
            { actor: getStringValue(payload.actor, "System Administrator"), documentName: getStringValue(payload.documentName) },
          );
          sendJson(res, 200, result);
          return;
        }

        if (segments[2] === "documents" && segments[3] === "regenerate") {
          const result = await enterpriseAutomationLayerService.regenerateDocumentDraft(getStringValue(payload.documentId), {
            actor: getStringValue(payload.actor, "System Administrator"),
          });
          sendJson(res, 200, result);
          return;
        ...
        if (segments[2] === "documents" && segments[3] === "approve") {
          const result = await enterpriseAutomationLayerService.approveDocumentDraft(
            getStringValue(payload.documentId),
            payload.userApproval === true,
            getStringValue(payload.actor, "System Administrator"),
          );
          sendJson(res, 200, result);
          return;
        }

        if (segments[2] === "documents" && segments[3] === "export") {
          const result = await enterpriseAutomationLayerService.exportDocument(
            getStringValue(payload.documentId),
            getStringValue(payload.format, "json"),
            getStringValue(payload.actor, "System Administrator"),
          );
          sendJson(res, 200, result);
          return;
        ...
        if (segments[2] === "knowledge" && segments[3] === "create") {
          const result = await enterpriseAutomationLayerService.createKnowledgeEntry(payload || {});
          sendJson(res, 200, result);
          return;
        }

        if (segments[2] === "knowledge" && segments[3] === "update") {
          const result = await enterpriseAutomationLayerService.updateKnowledgeEntry(getStringValue(payload.entryId), payload.changes || {});
          sendJson(res, 200, result);
          return;
        }

        if (segments[2] === "knowledge" && segments[3] === "supersede") {
          const result = await enterpriseAutomationLayerService.supersedeKnowledgeEntry(getStringValue(payload.entryId), payload.replacement || {});
          sendJson(res, 200, result);
          return;
        }

        if (segments[2] === "knowledge" && segments[3] === "relevant") {
          const result = await enterpriseAutomationLayerService.getRelevantKnowledge(payload.context || {});
          sendJson(res, 200, { results: result });
          return;
        }

        if (segments[2] === "forecast" && segments[3] === "scenarios") {
          const assumptions = payload.assumptions && typeof payload.assumptions === "object" ? payload.assumptions : {};
          const years = Array.isArray(payload.years) ? payload.years : [1, 3, 5, 10];
          const properties = await readPortfolioFile();
          const result = enterpriseAutomationLayerService.buildForecastScenarios({ assumptions, years, properties });
          sendJson(res, 200, result);
          return;
        }

        if (segments[2] === "forecast" && segments[3] === "confidence") {
          const assumptions = payload.assumptions && typeof payload.assumptions === "object" ? payload.assumptions : {};
          const properties = await readPortfolioFile();
          const result = {
            confidence: enterpriseAutomationLayerService.calculateForecastConfidence({ assumptions, properties }),
            warnings: enterpriseAutomationLayerService.generateForecastWarnings({ assumptions, properties }),
          };
          sendJson(res, 200, result);
          return;
        }

        if (segments[2] === "workflow" && segments[3] === "preview") {
          const result = await enterpriseAutomationLayerService.previewWorkflowTransition(getStringValue(payload.type), getStringValue(payload.sourceId));
          sendJson(res, 200, result);
          return;
        }

        if (segments[2] === "workflow" && segments[3] === "execute") {
          const result = await enterpriseAutomationLayerService.executeWorkflowTransition(
            getStringValue(payload.type),
            getStringValue(payload.sourceId),
            payload.userApproval === true,
            getStringValue(payload.actor, "System Administrator"),
          );
          sendJson(res, 200, result);
        ...
        if (segments[2] === "workflow" && segments[3] === "rollback") {
          const result = await enterpriseAutomationLayerService.rollbackWorkflowTransition(
        ...
        if (segments[2] === "diagnostics" && segments[3] === "run") {
          const result = await enterpriseAutomationLayerService.runSystemDiagnostics();
          sendJson(res, 200, result);
          return;
        }
      }

      sendJson(res, 404, { error: "Enterprise endpoint not found" });
      return;
    }

    if (segments[0] === "api" && segments[1] === "search" && req.method === "GET") {
      const auth = authorizeRequest(req, res, "read");
      if (!auth.allowed) return;
      const query = getStringValue(url.searchParams.get("q") || "");
      const entityType = getStringValue(url.searchParams.get("entityType") || "");
    ...
    if (segments[0] === "api" && segments[1] === "comps") {
      if (segments[2] === "operations") {
        ...
        if (req.method === "GET" && operationName === "templates") {
          const auth = authorizeRequest(req, res, "read");
          if (!auth.allowed) return;
          sendJson(res, 200, compOperationsService.listTemplates());
          return;
        }
        if (req.method === "POST" && operationName === "templates") {
          const auth = authorizeRequest(req, res, "write");
          if (!auth.allowed) return;
          let payload = {};
          try {
          ...
          sendJson(res, 200, compOperationsService.saveTemplate(payload));
          return;
        }
        if (req.method === "GET" && operationName === "search-history") {
          const auth = authorizeRequest(req, res, "read");
          if (!auth.allowed) return;
          sendJson(res, 200, compOperationsService.listSearchHistory());
          return;
        }
        if (req.method === "POST" && operationName === "search-history") {
          const auth = authorizeRequest(req, res, "write");
          if (!auth.allowed) return;
          let payload = {};
          try {
          ...
          if (segments[4] === "archive") {
            sendJson(res, 200, compOperationsService.archiveSearch(payload.searchId || payload.id, payload.reason || "Archived", payload.actor || "System Administrator"));
            return;
          }
          if (segments[4] === "restore") {
            sendJson(res, 200, compOperationsService.restoreSearch(payload.searchId || payload.id, payload.reason || "Restored", payload.actor || "System Administrator"));
            return;
          }
          sendJson(res, 200, compOperationsService.recordSearch(payload));
          return;
        }
        if (req.method === "POST" && operationName === "freshness") {
          const auth = authorizeRequest(req, res, "read");
          if (!auth.allowed) return;
          let payload = {};
          try {
          ...
          sendJson(res, 200, compOperationsService.evaluateFreshness(payload.comp || payload));
          return;
        }
        if (req.method === "POST" && operationName === "refresh-queue") {
          const auth = authorizeRequest(req, res, "write");
          if (!auth.allowed) return;
          let payload = {};
          try {
          ...
          sendJson(res, 200, compOperationsService.queueRefresh(payload));
          return;
        }
        if (req.method === "POST" && operationName === "lifecycle") {
          const auth = authorizeRequest(req, res, "write");
          if (!auth.allowed) return;
          let payload = {};
          try {
          ...
          sendJson(res, 200, compOperationsService.transitionLifecycle(payload));
          return;
        }
        if (req.method === "POST" && operationName === "duplicates") {
          const auth = authorizeRequest(req, res, "read");
          if (!auth.allowed) return;
          let payload = {};
          try {
          ...
          sendJson(res, 200, compOperationsService.detectDuplicates(payload.comps || []));
          return;
        }
        if (req.method === "POST" && operationName === "conflicts") {
          const auth = authorizeRequest(req, res, "write");
          if (!auth.allowed) return;
          let payload = {};
          try {
          ...
          sendJson(res, 200, compOperationsService.createConflict(payload));
          return;
        }
        if (req.method === "POST" && operationName === "bulk") {
          const auth = authorizeRequest(req, res, "write");
          if (!auth.allowed) return;
          let payload = {};
          try {
          ...
          sendJson(res, 200, compOperationsService.applyBulkAction(payload));
          return;
        }
        if (req.method === "POST" && operationName === "readiness") {
          const auth = authorizeRequest(req, res, "read");
          if (!auth.allowed) return;
          let payload = {};
          try {
          ...
          sendJson(res, 200, compOperationsService.evaluateReadiness(payload));
          return;
        }
        if (req.method === "POST" && operationName === "diagnostics") {
          const auth = authorizeRequest(req, res, "read");
          if (!auth.allowed) return;
          let payload = {};
          try {
          ...
          sendJson(res, 200, compOperationsService.buildDiagnostics(payload));
          return;
        }
        if (req.method === "GET" && operationName === "protected-facts") {
          const auth = authorizeRequest(req, res, "read");
          if (!auth.allowed) return;
          sendJson(res, 200, compOperationsService.getProtectedFacts());
          return;
        }
        sendJson(res, 404, { error: "Comp operations endpoint not found" });
        return;
      }
      ...
      if (req.method === "POST" && segments[2] === "subject-property") {
        ...
        const searchSession = providerSearchSessionService.createSession({ provider: activeProviderKey, operation: "subject-property", query: { address }, status: "running" });
        const cacheKey = `subject-property:${String(activeProviderKey).toLowerCase()}:${address.toLowerCase()}`;
        const cachedResult = providerSearchSessionService.getCachedResult(cacheKey);
        if (cachedResult) {
          providerSearchSessionService.recordUsage(activeProviderKey, "subject-property", "cached");
          providerSearchSessionService.recordResult(searchSession.id, { status: "Cached", resultCount: 1, cachedResults: true, requestDurationMs: 0, lastError: "" });
          sendJson(res, 200, { ok: true, status: "Cached", errorCode: null, property: cachedResult.property || null, cached: true, sessionId: searchSession.id });
          return;
        }
        ...
        if (subjectResult.ok && subjectResult.property) {
          providerSearchSessionService.setCachedResult(cacheKey, responsePayload);
        }
        providerSearchSessionService.recordUsage(activeProviderKey, "subject-property", subjectResult.ok ? "completed" : "failed");
        providerSearchSessionService.recordResult(searchSession.id, { status: subjectResult.status, resultCount: subjectResult.property ? 1 : 0, cachedResults: false, requestDurationMs: 0, lastError: subjectResult.errorCode || "" });
      ...
      if (req.method === "POST" && segments[2] === "active-listings") {
        ...
        const providerResults = await multiSourceCompSearchService.searchActiveListings(query);
        const records = Array.isArray(providerResults.records) ? providerResults.records : [];
        addProviderSearchHistory({ operation: "active-listings", query, ok: true, status: records.length > 0 ? "Success" : "No Results", resultCount: records.length });
        sendJson(res, 200, { ok: true, status: records.length > 0 ? "Success" : "No Results", errorCode: null, records, resultCount: records.length, providerSummary: providerResults.providerResults || [] });
        return;
      }
      ...
      if (req.method === "GET" && segments[2] === "search-session") {
        sendJson(res, 200, providerSearchSessionService.getSummary());
        return;
      }
      if (req.method === "POST" && segments[2] === "search-session") {
        let payload = {};
        try {
          payload = await readJsonBody(req);
        } catch (error) {
        ...
        if (action === "cancel" && sessionId) {
          const session = providerSearchSessionService.cancelSession(sessionId);
          sendJson(res, 200, { ok: Boolean(session), session });
          return;
        }
        if (action === "retry" && sessionId) {
          const session = providerSearchSessionService.retrySession(sessionId);
          sendJson(res, 200, { ok: Boolean(session), session });
          return;
        }
        if (action === "snapshot" && sessionId) {
          const session = providerSearchSessionService.snapshotSession(sessionId, payload.snapshot || {});
          sendJson(res, 200, { ok: Boolean(session), session });
          return;
        }
    ...
    if (segments[0] === "api" && segments[1] === "portfolio") {
      if (segments[2] === "deal-sync") {
        if (req.method === "GET" && segments[3] === "eligible") {
          const auth = authorizeRequest(req, res, "read");
          if (!auth.allowed) return;
          const eligibleDeals = await dealPortfolioSyncService.listEligibleDeals();
          sendJson(res, 200, { eligibleDeals });
          return;
        }
        ...
        if (req.method === "GET" && segments[3] === "audit") {
          const auth = authorizeRequest(req, res, "read");
          if (!auth.allowed) return;
          const auditEvents = await dealPortfolioSyncService.listAuditEvents();
          sendJson(res, 200, { auditEvents });
          return;
        }

        if (req.method === "GET" && segments[3] === "status") {
          const auth = authorizeRequest(req, res, "read");
          if (!auth.allowed) return;
          const dealId = url.searchParams.get("dealId") || "";
          const status = await dealPortfolioSyncService.getDealPortfolioSyncStatus(dealId);
          sendJson(res, 200, status);
          return;
        }
        ...
        if (req.method === "POST") {
          ...
          if (segments[3] === "preview") {
            const preview = await dealPortfolioSyncService.previewDealToPortfolioSync(dealId, actor);
            sendJson(res, 200, preview);
            return;
          }

          if (segments[3] === "execute") {
            const result = await dealPortfolioSyncService.executeDealToPortfolioSync(dealId, payload.userApproval === true, actor);
            if (result?.ok && safeString(result?.status) !== "REVIEW_REQUIRED_DUPLICATE") {
              await crossModuleSyncService.synchronizeDealImport(dealId, actor);
            }
            sendJson(res, 200, result);
            return;
          ...
          if (segments[3] === "reconcile") {
            const portfolioId = getStringValue(payload.portfolioId);
            const result = await dealPortfolioSyncService.reconcileDealAndPortfolio(dealId, portfolioId, actor);
            sendJson(res, 200, result);
            return;
          }

          if (segments[3] === "rollback") {
            const result = await dealPortfolioSyncService.rollbackLastSyncEvent(dealId, actor);
            sendJson(res, 200, result);
            return;
          }
        }

        sendJson(res, 404, { error: "Deal sync endpoint not found" });
        return;
      }
...
for (const signal of shutdownSignals) {
  process.on(signal, async () => {
    ...
    if (writerLockHandle) {
      try {
        await writerLockHandle.close();
      } catch {
        // ignore lock close failures during shutdown
      }
      await fs.rm(path.join(dataDir, ".rsos-writer.lock"), { force: true }).catch(() => {});
    }
