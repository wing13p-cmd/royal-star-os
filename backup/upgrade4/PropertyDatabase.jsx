...
const statusOptions = ["Lead", "Under Review", "Offer Submitted", "Under Contract", "Due Diligence", "Closed", "In Rehab", "Ready to Rent", "Rented", "Listed", "Under Contract to Sell", "Sold", "On Hold", "Cancelled", "Archived"];
...
export default function PropertyDatabase({
  ...
  const [missingParcelFilter, setMissingParcelFilter] = useState("All");
  const [missingSourceFilter, setMissingSourceFilter] = useState("All");
  const [staleDataFilter, setStaleDataFilter] = useState("All");
  ...
  const countyOptions = useMemo(() => ["All", ...Array.from(new Set(normalizedProperties.map((property) => property.county).filter(Boolean))).sort()], [normalizedProperties]);
  ...
  const occupancyOptionsList = useMemo(() => ["All", ...occupancyOptions], []);
  ...
  const persistProperty = async (payload, existingProperty = null) => {
    if (existingProperty) {
      try {
        const response = await fetch(`${API_BASE_URL}/api/properties/${existingProperty.id}`, {
          method: "PUT",
  ...
  const handleDelete = async (propertyId) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/properties/${propertyId}`, { method: "DELETE" });
  ...
  const handleDuplicate = () => {
    ...
    if (!window.confirm(`Create a duplicate of ${selectedProperty.propertyName || selectedProperty.address || "this property"}?`)) return;
    const duplicateValues = {
      ...
      createdAt: "",
      updatedAt: "",
      propertyStatus: selectedProperty.propertyStatus || selectedProperty.status || "Lead",
      favorite: Boolean(selectedProperty.favorite),
      propertyName: `${selectedProperty.propertyName || selectedProperty.address || "Property"} Copy`,
    };
  ...
  const handleToggleFavorite = (propertyId) => {
    const nextProperties = properties.map((property) => (property.id === propertyId ? { ...property, favorite: !Boolean(property.favorite) } : property));
    setProperties(nextProperties);
    if (typeof window !== "undefined") {
      window.localStorage.setItem("royalStarProperties", JSON.stringify(nextProperties));
    }
  ...
  const handleExport = () => {
    ...
    link.href = url;
    link.download = "royal-star-properties.json";
    link.click();
        ...
        <div style={styles.contentGrid}>
          <section style={styles.panel}>
            <div style={styles.panelHeaderRow}>
              ...
              <div style={styles.inlineActions}>
                ...
                <button type="button" style={styles.primaryButton} onClick={handleDuplicate} disabled={!selectedProperty}>DUPLICATE</button>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} style={styles.formGrid}>
              ...
              <label style={styles.field}><span>Status</span><select name="propertyStatus" value={formValues.propertyStatus} onChange={handleFieldChange} style={styles.select}>{statusOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>
              <label style={styles.field}><span>Pipeline Stage</span><input name="pipelineStage" value={formValues.pipelineStage} onChange={handleFieldChange} style={styles.input} /></label>
              ...
              <div style={{ gridColumn: "1 / -1", display: "flex", gap: 12 }}>
                <button type="submit" style={styles.primaryButton}>SAVE PROPERTY</button>
          ...
          <section style={styles.panel}>
            <div style={styles.panelHeaderRow}>
              ...
              <div style={styles.inlineActions}>
                <span style={styles.statusBadge}>{connectionState}</span>
              </div>
            </div>
                      ...
                      <tr key={property.id} style={styles.tr}>
                        ...
                        <td style={styles.td}>
                          <div style={styles.inlineActions}>
                            ...
                            <button type="button" style={styles.secondaryButton} onClick={() => handleDuplicate(property.id)}>Dup</button>
